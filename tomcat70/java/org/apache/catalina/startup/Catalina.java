...
public class Catalina {
    ...
    public void load() {
        ...
        // Create and execute our Digester
        //(Maj)使用Digester组件解析Tomcat的server.xml，
        //(Maj)初始化各个组件（包含各个web应用，解析对应的web.xml进行初始化）；
      //(Maj)初始化Digester组件，定义了解析规则
        Digester digester = createStartDigester();

        //主要作用为将configFile-->server.xml文件转换为输入流
        ...
        try {
            ...
            try {
                ...
                //(Maj)通过Digester解析这个文件，在此过程中会初始化各个组件实例及其依赖关系
                digester.push(this);
        ...
        // //(Maj) 调用Server的initialize方法，初始化各个组件
